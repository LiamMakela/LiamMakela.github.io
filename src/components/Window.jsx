...
function Window({ open , Content, closeWindow, width, height}) {
  ...
  return (
    <Draggable cancel=".no-drag" bounds="parent" nodeRef={nodeRef} defaultPosition={{ x: window.innerWidth / 2 - 128, y: window.innerHeight / 2 - 128}} >
      <div
        ref={nodeRef}
      ...
      >
        <Content/>
         <button onClick = {closeWindow} className=" no-drag cursor-pointer absolute top-0.5 right-0.5  text-white rounded w-7 h-7">
